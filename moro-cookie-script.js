/* -------------------------
   Cookie Banner MORO
   v2.5.8 (Deep-Glow & Scroll-Lock — Vollstabilisiert)
   ------------------------- */
...
function initCookieIframes() {
  ...
  // 1. 🔍 START UNIVERSAL-DIAGNOSE (Wird jetzt dank Hoisting fehlerfrei gefunden)
  runMoroDiagnostics();

  const consentTime = localStorage.getItem('cookieConsentTime');
  const expirationPeriod = 180 * 24 * 60 * 60 * 1000; 

  // Statische iFrames durch Platzhalter ersetzen
  ...
  if (consent === 'true') {
    ...
    // Aktiviert die Scroll-Sperre, falls block-scroll="true" am Banner gesetzt ist
  ...
  /* -------------------------
     GTM & MUTATION OBSERVER INNER FUNCTIONS
     ------------------------- */
      ...
      mutation.addedNodes.forEach(function(node) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          targetIframes.forEach(function(iframe) {
            ...
            if (!category) category = 'nicht-definiert';
  ...
  /* -------------------------
     HELPER INNER FUNCTIONS (Sicher deklariert gegen ReferenceErrors)
     ------------------------- */
  ...
  function buildWarningPopup(errors) {
    ...
    let popupContent = `<h2 style="margin-top:0; color:#d93838; font-size:20px; border-bottom:2px solid #fbcbcb; padding-bottom:10px; display:flex; align-items:center; gap:8px;">🛑 MORO Cookie Banner — Diagnose-Warnung</h2>`;
    errors.forEach(err => {
      popupContent += `<div style="background:#ffffff; border-left:5px solid #d93838; padding:15px; margin-bottom:20px; border-radius:4px;"><strong style="background:#d93838; color:#fff; padding:2px 6px; font-size:11px; border-radius:3px; text-transform: uppercase;">${err.type}</strong><p style="margin:8px 0; font-weight:bold; font-size:14px; color:#c62828;">${err.message}</p></div>`;
    });
    popupContent += `<div style="text-align:right;"><button id="moro-close-diag" style="background:transparent; border:none; text-decoration:underline; color:#666; cursor:pointer; font-size:13px;">Schließen</button></div>`;
    popup.innerHTML = popupContent;
    document.body.appendChild(popup);
    document.getElementById('moro-close-diag').addEventListener('click', function() { popup.remove(); });
  }
  ...
  function createPlaceholder(el, src, width, height, altImg, category) {
    ...
    // Geometrie-Fix & Deep Glow Design-Zuweisung
...
/* -------------------------
   GLOBAL WEBFLOW CHECKBOX HELPERS (Sicher außerhalb platziert)
   ------------------------- */
